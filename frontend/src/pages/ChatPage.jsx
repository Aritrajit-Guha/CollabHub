...
export default function ChatPage() {
  ...
  const joinRoom = () => {
    ...
    socketRef.current?.emit('joinRoom', { room: nextRoom, userName: nextUserName })
    setMessages((current) => [...current, { user: 'System', message: `Joined room: ${nextRoom} as ${nextUserName}` }])
  }
  ...
  const sendMessage = async () => {
    ...
    setMessages((current) => [...current, { user: currentUser, message: nextMessage }])
    setMessage('')
    ...
    try {
      const response = await fetch(apiUrl('/api/chat'), {
        ...
        body: JSON.stringify({ message: nextMessage, room: currentRoomRef.current, userName: currentUser }),
      })
      await readJsonResponse(response)
    } catch (error) {
      console.error(error)
      setWaitingForAi(false)
      setMessages((current) => [...current, { user: 'System', message: 'AI could not respond.', type: 'error-msg' }])
    }
  }
    ...
    <div id="chatContainer">
      <header id="chatHeader">
        ...
        <div id="roomJoin">
          ...
          <button type="button" id="joinBtn" onClick={joinRoom}>Join Room</button>
        </div>
      </header>
        ...
        {messages.map((item, index) => {
          ...
          return (
            <div className={`message ${type}`} key={`${item.user}-${index}`}>
              ...
              {type === 'ai-msg' ? <ReactMarkdown>{item.message}</ReactMarkdown> : <div>{type === 'system-msg' ? <em>{item.message}</em> : item.message}</div>}
            </div>
          )
        })}
        {waitingForAi && <div className="message ai-msg"><em>🤖 CollabAI is typing...</em></div>}
      </main>
